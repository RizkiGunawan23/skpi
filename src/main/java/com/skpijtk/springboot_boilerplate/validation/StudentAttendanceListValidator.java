...
public class StudentAttendanceListValidator {
    public static List<FieldErrorResponse> validate(StudentAttendanceListQuery query) {
        List<FieldErrorResponse> errors = new ArrayList<>();

        // Validasi student_id
        if (query.getStudent_id() != null && !query.getStudent_id().isBlank()) {
            try {
                Long.valueOf(query.getStudent_id());
            } catch (NumberFormatException e) {
                errors.add(new FieldErrorResponse("student_id", "student_id must be a valid number"));
            }
        }
