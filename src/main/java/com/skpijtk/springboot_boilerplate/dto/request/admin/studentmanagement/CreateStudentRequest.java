...
public class CreateStudentRequest {
    ...
    @Size(min = 6, max = 12, message = "Password must be between {min} and {max} characters")
    @Pattern(regexp = "^(?=.*[A-Za-z])(?=.*\\d)[A-Za-z\\d]{6,12}$", message = "Password must contain both letters and numbers, no special characters or spaces")
